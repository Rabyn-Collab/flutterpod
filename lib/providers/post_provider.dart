...
final postProvider = StateNotifierProvider<PostProvider, CommonState>((ref) => PostProvider(
    CommonState(
        errMessage: '',
        isError: false,
        isLoad: false,
...
class PostProvider extends StateNotifier<CommonState>{
  PostProvider(super.state);
   ...
   Future<void> postAdd({
    ...
    final response = await PostService.postAdd(title: title, detail: detail, userId: userId, image: image);
    response.fold(
            (l) {
